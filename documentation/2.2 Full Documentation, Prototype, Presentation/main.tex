...
\usepackage{tabularx}
\usepackage{graphicx}
\graphicspath{{./PrototypeScreenshots/}}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item Core authentication and authorization funtionality should be updated in the prototype. Email verification should automatically be set to verified upon account creation. KYC should use a force-pass route to quickly authorize a user. Finally, the duo feature will not be included in the prototype.
...
As a \textbf{Bidder}, I want to place a bid with an amount, note, and ETA so the Job Poster/Contractor can evaluate me.
\begin{itemize}[leftmargin=1.4em]
...
As a \textbf{Bidder}, I want to verify my identity (document + selfie) so I can browse and bid on jobs.
\begin{itemize}[leftmargin=1.4em]
...
As a \textbf{Bidder}, I want Duo phone confirmation so my account is protected during sign-in.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} 2FA challenge required; recovery path documented.
\end{itemize}

\paragraph{US-5: Job Thread Messaging (Medium, 3 pts)}
As a \textbf{Bidder}, I want a per-job chat so I can coordinate details with job posters/contractors.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Send/receive text; attachments; report/block; notifications.
\end{itemize}

\paragraph{US-6: Leave Reviews (Medium, 2 pts)}
As a \textbf{Bidder}, I want to rate and review job posters after completion so future users can trust matches.
\begin{itemize}[leftmargin=1.4em]
...
As a \textbf{Bidder}, I want to create an account using my email.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Login page present with a seamless sign up process.
\end{itemize}

\paragraph{US-8: Personailze Profile (High, 5 pts)}
As a \textbf{Bidder}, I want to personalize my profile to showcase my skills, reviews, experience etc.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Editable user profile page to change profile picture, skills, experience etc.
\end{itemize}

\subsection*{Job Posters/Contractors}
...
As a \textbf{Contractor}, I want a per-job chat so I can coordinate details with Job Bidders.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Send/receive text; attachments; report/block; notifications.
\end{itemize}

\paragraph{US-14: Leave Reviews (Medium, 2 pts)}
As a \textbf{Contractor}, I want to rate and review Job Bidders after completion so future users can trust contractors.
\begin{itemize}[leftmargin=1.4em]
...
\subsection*{Job Browsers (Non KYC-Verified Users)}
...
As a \textbf{browser}, I want to view limited job details (e.g., title, category, approximate location, budget range) without completing the KYC process, so I see the platform's value.
\begin{itemize}[leftmargin=1.4em]
...
\paragraph{US-18: KYC/2FA Instructions (Medium, 1 pt)}
...
\paragraph{US-19: Access Support Resources (Low, 1 pt)}
As a \textbf{browser}, I want access to support or FAQ pages, so I can ask questions or learn more about the app before starting the KYC verification process.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Support contact form; FAQ section; help documentation accessible without being KYC-verified.
\end{itemize}

\subsection*{Admin}
\paragraph{US-20: Review Reports (High, 3 pts)}
...
\paragraph{US-21: Manage Categories (Medium, 2 pts)}
...
\paragraph{US-22: Handle Disputes (High, 4 pts)}
...
\paragraph{US-23: System Monitoring (Medium, 2 pts)}
...
\begin{enumerate}[leftmargin=1.4em]
  \item \textbf{Account \& Identity (\textrightarrow{} US-3, US-7, US-11, US-15):} Email/password sign-up and sign-in (handled via Firebase Auth in production). Mandatory KYC before bidders can bid or posters can create jobs (handled via Stripe Identity in production).
  \item \textbf{Viewing Jobs (\textrightarrow{} US-16, US-17):} Job browsers can view jobs after signing up an account.
  \item \textbf{Bidder Profiles (\textrightarrow{} US-8):} Bidder can create and update basic profiles. In production, bidders can enhance their profiles by including skills, description, photos, rates, availability, and their location. Support for deletion.
  \item \textbf{Discovery (\textrightarrow{} US-1):} Bidders can browse for jobs using a list. In production, there will be an added map view and job filters (radius, category/skill, budget, availability date).
  \item \textbf{Bidding (\textrightarrow{} US-2):} KYC verified bidders can place, edit, and withdraw bids (amount, note, ETA).
  \item \textbf{Posting Jobs (\textrightarrow{} US-10):} KYC verified Job Posters can place, edit, and delete jobs (amount, location, description, etc.). A Job Poster can additionally accept a bid on a job.
  ...
  \item \textbf{Admin Dashboard: (\textrightarrow{} US-20, US-21, US-22, US-23)} Admins manage reports, categories, and disputes.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item \textbf{Usability:} Mobile-first, accessible (WCAG 2.1 AA where practical); clear copy for KYC/2FA and escrow steps. Map and list view of jobs should be easily accessible on any device. The application shall include an FAQ page, support resources, and clear messages to indicate KYC verification and Duo is required to use the app (addresses US-18 and US-19 which are not covered by current FRs).
...
NFR-004 & \textbf{Privacy:} System shall provide an approximate job location during the pre-accept phase. Accepted bidders will be granted access to the full job location. & TC-023 & Planned \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item At this time, it was determined that it would be difficult to implement tests for the scalability and reliability non-FRs. Test cases for these non-FRs will be planned and implemented in a future iteration.
\end{itemize}
...
\begin{tabularx}{\linewidth}{|l|L|}
\hline
\textbf{Test Case ID} & TC-023 \\ \hline
\textbf{Title} & Privacy - Job Location Masking Until Acceptance \\ \hline
\textbf{Requirement} & NFR-004 \\ \hline
\textbf{Preconditions} & Logged in, KYC verified as a bidder. \\ \hline
\textbf{Steps} &
\begin{minipage}[t]{\linewidth}
\begin{enumerate}[leftmargin=1.4em,nosep,topsep=0pt]
  \item Job bidder views job with location masked.
  \item Job bidder bids on job.
  \item Job bidder is accepted for a job by a job poster.
  \item Job location is revealed to the job bidder.
\end{enumerate}
\end{minipage} \\ \hline
\textbf{Expected Result} & Full job location revealed to job bidder after acceptance. \\ \hline
\textbf{Actual Result} & To be filled after execution. \\ \hline
\textbf{Status} & Planned \\ \hline
\textbf{Priority} & High \\ \hline
\end{tabularx}
...
To run the prototype, please refer to the Quick Start Guide in the project's README file.  
Follow the steps under the \textbf{Running in Prototype Mode} section.  
These instructions will walk you through starting the application for the first time.

\subsection{Using the Prototype}

\subsubsection{Using OpenBid as a Job Browser}
\begin{enumerate}[leftmargin=1.4em,nosep,topsep=0pt]
  \item \textbf{Log In or Create an Account.}  
  When you open the application, you will see the login page.  
  If you do not have an account yet, click the \textit{Create Account} button and follow the steps to register. Then log in.
  
  \item \textbf{Browse Job Listings.}  
  Once logged in, you can view all available job postings created by job posters.  
  At this stage, you can only look at job listings unless you complete KYC (see below).

  \item \textbf{Update Your Profile Picture (Optional).}  
  Click the profile icon located on the right side of the header.  
  On the profile page, click \textit{Change Avatar} to upload a new profile picture.
\end{enumerate}

\subsubsection{KYC Verification}
\textbf{You must complete KYC (identity verification) to create jobs or place bids. To complete KYC:}
\begin{enumerate}[leftmargin=1.4em,nosep,topsep=0pt]
  \item Go to your profile by clicking the profile button in the top-right corner.
  \item Click the \textit{Complete KYC} button and follow the instructions.  
        After completing KYC, you will gain access to both bidding and job creation features.
\end{enumerate}

\subsubsection{Using OpenBid as a Job Bidder}
\begin{enumerate}[leftmargin=1.4em,nosep,topsep=0pt]
  \item After logging in, open the job list page and click the \textit{I want to bid on jobs} button.
  \item Browse the list of available jobs and click on any job to view its details.
  \item If the job is open, you may submit a bid. You can also include an optional note with your bid.
  \item You may update your bid at any time before the job closes.
  \item If your bid is accepted by the job poster, the job will close automatically.  
        When you revisit the job page, you will see a notice indicating that your bid was accepted.
\end{enumerate}

\subsubsection{Using OpenBid as a Job Poster}
\begin{enumerate}[leftmargin=1.4em,nosep,topsep=0pt]
  \item After logging in. From the job list page, click the \textit{I want to post jobs} button.
  \item Click the \textit{Post a Job} button to begin creating a new job posting.
  \item Fill out all required details for the job and press \textit{Create} to publish it.
  \item You may update or delete one of your jobs by selecting it from the job list after choosing \textit{I want to post jobs}.
  \item As bidders submit offers, you can view them by opening your job post.  
        When ready, select a single bid to accept.
  \item Once a bid is accepted, the job will close. The bidder whose offer you selected will see a notice confirming that their bid was chosen.
\end{enumerate}

\subsubsection{*Using OpenBid with a dev account}
When testing the application as a developer, use the following credentials to bypass the account creation process and log in directly.
\begin{itemize}[leftmargin=1.4em,nosep,topsep=0pt]
  \item \textbf{Email:} openbid123@gmail.com
  \item \textbf{Password:} openbidopenbid
\end{itemize}

\section{Prototype Overview}
\subsection{Protoype Description}
The prototype highlights the core functionality of the OpenBid Application. Authenticated users can browse job postings from job posters as
a job browser. Users that complete KYC verification can additionally bid on jobs as a job bidder and create jobs as a job poster. The general user journey for a 
job bidder is the user first browses for jobs, then bids on a job, then the bid is either accepted or rejected (if the job closes or another bidder's bid is selected). The general user journey
for a job poster is the job poster first posts a job, makes optional edits, receives bids for the jobs, then closes the job at any time or accepts a bid from a bidder.

\subsection{Prototype Screenshots}
\textbf{Note that some screenshots are intentionally cut off due to sizing restrictions. Most importantly, the main components for each page are included in each image.}
\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{MainLogin}
    \caption{Main Login Page}
    \label{fig:MainLoginImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{CreateAccount}
    \caption{Create Account Page}
    \label{fig:CreateAccountImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{ForgotPassword}
    \caption{Forgot Password Page}
    \label{fig:ForgotPasswordImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{ProfileBeforeKYC}
    \caption{User profile page before KYC verification}
    \label{fig:ProfileBeforeKYCImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{ProfileAfterKYC}
    \caption{User profile page after KYC verification}
    \label{fig:ProfileFAfterKYCImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobListBidder}
    \caption{Main job list page as a Job Bidder}
    \label{fig:JobListBidderImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobBidderBidding}
    \caption{Page when a Job Bidder selects a job to bid on.}
    \label{fig:BiddingImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{BidderBids}
    \caption{Job Bidder's bids page}
    \label{fig:BidsAsBidderImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobBidderBidding}
    \caption{Page after a Job Bidder bids on a job.}
    \label{fig:AfterBiddingImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobBidAccepted}
    \caption{Page after a Job Bidder's bid is accepted for a job.}
    \label{fig:JobBidAcceptedImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobListPoster}
    \caption{Main job list page as a Job Poster}
    \label{fig:JobPosterImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{JobListPosterKYC}
    \caption{Main job list page when a Job Poster is KYC verified}
    \label{fig:JobPosterKYCImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{PostJob}
    \caption{Posting a job page}
    \label{fig:PostingJobImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{ViewingJobAsJobPoster}
    \caption{Job poster viewing a created job page}
    \label{fig:ViewingJobAsJobPosterImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{JobBids}
    \caption{Job poster viewing a created job page with bids}
    \label{fig:JobBidsImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{AfterAcceptingBid}
    \caption{Job poster job page after accepting a bid}
    \label{fig:AfterAcceptingBidImg}
\end{figure}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{RestrictedBidding}
    \caption{Page displayed when a user attempts to place a bid on their own job posting}
    \label{fig:RestrictedBiddingImg}
\end{figure}
...
% ===================== 9. Next Steps =====================
\section{\textbf{Next Steps:} Bulding and Testing the Production application (MVP)}
\begin{enumerate}
  \item \textbf{Future Refined Stack (MVP):} The application will be migrated to the following stack: React + SCSS (Firebase Hosting), Node.js + Express (Firebase Functions or Cloud Run), Firestore, Google Maps JS, Stripe (Connect/Identity), Duo 2FA.
  \item \textbf{Testing:} Prototype tests that are incompatible with production features will be updated while new tests will be created to test production level features.
  \item \textbf{User Feedback:} The prototype and eventual production application will be updated based on feedback from users.
\end{enumerate}

% ===================== 10. Submission Guidelines =====================
\section{Submission Guidelines}
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{GitHub:} Push code and documentation; tag release as \texttt{ITR2.2}.
  \item \textbf{eClass PDF:} include \textit{Prototype Overview with Screenshots}, \textit{Updated Requirements/Use Cases}, \textit{Testing Document}, and \textit{User Manual}.
\end{itemize}
