...
\paragraph{US-8: Create an account (High, 5 pts)}
...
\paragraph{US-9: Accept a Bid \& Fund Escrow (High, 4 pts)}
...
As a \textbf{Job Poster}, I want to rate and review Contractor after completion so future users can trust matches.
\begin{itemize}[leftmargin=1.4em]
...
\paragraph{US-16: Accept a Bid \& Fund Escrow (High, 4 pts)}
As a \textbf{Job Poster}, I want to accept a bid and put funds on hold so payment is guaranteed on completion.
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{Acceptance:} Stripe hold succeeds; job moves to \texttt{awarded}; chat thread opens with winner.
\end{itemize}
...
\paragraph{US-17: Browse Jobs Read-Only (Medium, 2 pts)}
...
\paragraph{US-18: View Limited Job Details (Medium, 1 pt)}
...
\paragraph{US-19: Registration Prompt (Medium, 1 pt)}
...
\paragraph{US-20: KYC/2FA Instructions (Medium, 1 pt)}
...
\paragraph{US-21: Access Support Resources (Low, 1 pt)}
...
\subsection*{Admin}
\paragraph{US-22: Review Reports (High, 3 pts)}
...
\paragraph{US-23: Manage Categories (Medium, 2 pts)}
...
\paragraph{US-24: Handle Disputes (High, 4 pts)}
...
\paragraph{US-25: System Monitoring (Medium, 2 pts)}
...
\begin{enumerate}[leftmargin=1.4em]
  \item \textbf{Account \& Identity:}  Email/password sign-up and sign-in (handled via Firebase Auth in production). Mandatory KYC before Contractors can bid or Posters can create jobs (handled via Stripe Identity in production).
  \item \textbf{Contractor Profiles:} Contractors can create and update basic profiles. In production, contractors can enhance their profiles by including skills, description, photos, rates, availability, and their location. Support for deletion.
  \item \textbf{Discovery:} Contractors can browse for jobs using a list. In production, there will be an added map view and job filters (radius, category/skill, budget, availability date).
  \item \textbf{Bidding:} KYC verified Contractors can place, edit, and withdraw bids (amount, note, ETA).
  \item \textbf{Posting Jobs:} KYC verified Posters can place, edit, and delete jobs (amount, location, description, etc.).
  \item \textbf{Two-Factor Authentication (2FA):} Duo 2FA required for sign-in.
  \item \textbf{Payments (Escrow):} Posters fund escrow on offer acceptance; payouts via Stripe Connect on completion.
  \item \textbf{Messaging:} Secure in-offer messaging with attachments and report/block options.
  \item \textbf{Reviews:} Both parties leave ratings and written reviews.
  \item \textbf{Safety Score:} System calculates a 0--100 location safety score and applies friction controls.
  \item \textbf{Admin Dashboard:} Admins manage reports, categories, and disputes.
\end{enumerate}
